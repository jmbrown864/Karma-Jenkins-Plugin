    ...
    if(this.maxCacheEntries > 0) {        
        ...
        // Look in local cache
            ...
            if(nCacheLength > 0) {
                ...
                // Function through each cached element
...
parseXMLResult : function(result) {
    ...
    try {
        // Function through each data field in each result using the schema
 ...
 */
 var Dt = {
    ...
    format : function (oDate, oConfig, sLocale) {
        ...
        // First replace aggregates (run in a function because an agg may be made up of other aggs)
        while(format.match(/%[cDFhnrRtTxX]/)) {
            format = format.replace(/%([cDFhnrRtTxX])/g, replace_aggs);
        }

        // Now replace formats (do not run in a function otherwise %%a will be replace with the value of %a)
