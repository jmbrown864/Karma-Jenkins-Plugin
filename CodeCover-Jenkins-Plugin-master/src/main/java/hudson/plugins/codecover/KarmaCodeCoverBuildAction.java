...
/**
 * Build view extension by KarmaCodeCover plugin.
 *
 * As {@link CoverageObject}, it retains the overall coverage report.
 *
 * @author Kohsuke Kawaguchi
 */
public final class KarmaCodeCoverBuildAction extends CoverageObject<KarmaCodeCoverBuildAction> implements HealthReportingAction, StaplerProxy {
     ...
     */
    private final KarmaCodeCoverHealthReportThresholds thresholds;

    public KarmaCodeCoverBuildAction(AbstractBuild<?,?> owner, Rule rule, Ratio statementCoverage, Ratio branchCoverage, Ratio loopCoverage, Ratio conditionCoverage, KarmaCodeCoverHealthReportThresholds thresholds) {
        this.owner = owner;
	...
	protected static File getKarmaCodeCoverReport(File file) throws IOException, InterruptedException {
		if (file.isDirectory()) {
			File report = null;
    ...
    @Override
    public KarmaCodeCoverBuildAction getPreviousResult() {
        return getPreviousResult(owner);
    }

    /**
     * Gets the previous {@link KarmaCodeCoverBuildAction} of the given build.
     */
    /*package*/ static KarmaCodeCoverBuildAction getPreviousResult(AbstractBuild<?,?> start) {
     ...
     */
    public static KarmaCodeCoverBuildAction load(AbstractBuild<?,?> owner, Rule rule, KarmaCodeCoverHealthReportThresholds thresholds, FilePath... files) throws IOException {
        Ratio ratios[] = null;
        ...
        return new KarmaCodeCoverBuildAction(owner,rule,ratios[0],ratios[1],ratios[2],ratios[3],thresholds);
    }

    public static KarmaCodeCoverBuildAction load(AbstractBuild<?,?> owner, Rule rule, KarmaCodeCoverHealthReportThresholds thresholds, InputStream... streams) throws IOException {
        Ratio ratios[] = null;
    ...
    private static Ratio[] loadRatios(InputStream in, Ratio[] r) throws IOException {
    	...
    	if (r == null || r.length < 4) { r = new Ratio[4] };
    ...
    private static String ratioFinder(String line, int start, int end) {
    	...
    	while (start < end) {
    		ratio = ratio + line.charAt(start);
    		start++;
    	}

    	if (ratio.matches("\d* \/ \d*") == false) {
    		return false;
    	}
    ...
    private static final Logger logger = Logger.getLogger(KarmaCodeCoverBuildAction.class.getName());
}
